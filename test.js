...
const copyObject = (obj) => {
  ...
  if (obj === undefined) return undefined;//return undefined for undefined value

  if (obj instanceof Date) {
    return new Date(obj); // return the new date with obj value
  }
  ...
  let finalObj = Object.create(obj);
  for (let key in obj) {
    if (obj.hasOwnProperty(key)) {
      if (obj[key] instanceof Object) {
        finalObj[key] = copyObject(obj[key]);
      } else {
